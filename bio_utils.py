            ...
            for words, starts, ends in get_token_spans(text):
                ...
                for res in x:
                    ...
                    try:
                        ...
                        if txt+entity not in raw_train:
                            raw_train[txt+entity] = (temp_w, entity, e_pos, [trigger_pos], [tlbl], pos, [rule], [rname])
                        else:
                            ...
                            raw_train[txt+entity][6].append(rule)
                            raw_train[txt+entity][7].append(rname)
                ...
                for entity in y:
                    try:
                        ...
                        pos_lang.addSentence(pos)
                        input_lang.addSentence(temp_w)
                        raw_train[txt+entity] = (temp_w, entity, e_pos, [-1], ["NoRel"], pos, [[]], [-1])
                    except:
                        continue
                        # print (words)
                        # print (new_words)
...
def prepare_data_from_json(jfile, input_lang, pl1, char, rule_lang, train):
    with open(jfile) as f:
        raw_train = json.load(f)
    
    for datapoint in raw_train:
        input_lang.addSentence(datapoint[0])
        pl1.addSentence(datapoint[5])
    ...
    for i, w in input_lang.index2word.items():
        char.addSentence(w)
    return input_lang, pl1, char, rule_lang, train + raw_train


def parse_json_data():
    triggers = dict()
    valids = dict()
    with open("lo_events_old.json") as f:
        pubmed = json.load(f)
        i = 0
...
# #     # print(input_lang.labels, input_lang.label2id)
# #     input2_lang, pl2, char2, rule_lang2, raw_test = prepare_data("BioNLP-ST-2013_GE_devel_data_rev3", valids="valids.json")
#     i = j = 0
# #     test = []
#     for datapoint in raw_train:
#         if datapoint[3][0] != -1:
#             i += len(datapoint[3])
#             try:
#                 test.append(([input_lang.word2index[w] if w in input_lang.word2index else 2 for w in datapoint[0]]+[0],
#                     datapoint[1],#entity
